...
void OrderLayOut::init_page(AppContext &ctx, std::function<void()> on_checkout,
    ...
    // 没有订单渲染界面
    if (orders_map_ptr->empty()) {

        // 底部导航按钮
        auto btn_to_shopping = Button("购物商城", on_shopping,
                                      ButtonOption::Animated(Color::Green));
        auto btn_to_cart = Button("返回购物车", on_checkout,
        ...
        auto btn_container = Container::Horizontal(
            {btn_to_cart, btn_to_shopping, btn_to_history});
        ...
        this->component = Renderer(btn_container, [=] {
            return vbox(
                {// 标题栏
                 ...
                 vbox({text(""), text("目前没有正在进行的订单") | center,
                       text("去商城逛逛吧！") | center}) |
    ...
    // 弹窗的基本样式，窗口标题模板
    auto MakeModal = [&](Component content, const string &title,
    ...
    auto popup_select_renderer = MakeModal(popup_select_layout, "提示", [=] {
        return vbox({text("修改订单") | center, separator(),
                     hbox({filler(), btn_opt_address->Render(), filler(),
                           btn_opt_delivery->Render(), filler(),
                           btn_opt_cancel->Render(), filler()})});
    });

    auto popup_address_renderer = MakeModal(popup_address_layout, "提示", [=] {
        return vbox({text("修改收货地址") | center, separator(),
                     input_address_comp->Render() | center, separator(),
                     vbox({hbox({filler(), btn_addr_auto->Render(), filler(),
                                 btn_addr_confirm->Render(), filler(),
                                 btn_addr_back->Render(), filler()}),
                           separator(),
                           text(status_text) | color(Color::Red) | center})});
    });

    auto popup_delivery_renderer =
        MakeModal(popup_delivery_layout, "提示", [=] {
            return vbox({text("修改快递方式") | center, separator(),
                         menu_delivery->Render() | center, separator(),
                         hbox({filler(), btn_delivery_confirm->Render(),
                               filler(), btn_delivery_back->Render(), filler()})

            });
        });

    auto popup_cancel_renderer = MakeModal(popup_cancel_layout, "提示", [=] {
        return vbox({text("确定要取消这个订单吗？") | center,
                     text("取消后无法恢复") | color(Color::Red) | center,
                     separator(),
                     hbox({filler(), btn_cancel_yes->Render(), filler(),
                           btn_cancel_no->Render(), filler()})});
    });

    // 简单弹窗模板
    auto MakeAlert = [&](Component btn, Component container) {
        return MakeModal(container, "提示", [=] {
            return vbox(
                {text(status_text) | color(Color::Green) | center, separator(),
                 btn->Render() | center | size(WIDTH, GREATER_THAN, 10)});
        });
    };

    auto popup_update_address_success_renderer = MakeAlert(
        btn_address_success_ok, popup_update_address_renderer_success_layout);

    auto popup_update_delivery_success_renderer = MakeAlert(
        btn_delivery_success_ok, popup_update_delivery_renderer_success_layout);
    ...
    // 重建订单列表
    rebuild_order_list_ui(ctx, main_container, orders_map_ptr, on_orders_update,
                          on_orders_delete);

    auto main_logic_content = Container::Vertical({scroller, btn_container});
...
// 重建 UI 列表
void OrderLayOut::rebuild_order_list_ui(
    ...
    for (auto it = orders_map_ptr->rbegin(); it != orders_map_ptr->rend();
        ...
        auto item_renderer = Renderer(card_controls, [&ctx, this, order_id,
            ...
            auto format_arrival_t = Utils::specific_hour_to_string(arrival_t);
            ...
            for (const auto &item : full_order.items) {
                auto prod = ctx.product_manager.get_product(item.product_id);
                std::string p_name =
                ...
                rows.push_back(hbox(
                    {text(" 商品名: "),
                     ...
                     filler(),
                     text("￥" + Utils::format_price(p_price * item.count)) |
                         color(Color::BlueLight)}));
            }
            rows.push_back(separator());
            ...
            rows.push_back(hbox(
                {text(" 配送方式: "), text(del_name) | color(Color::Cyan),
                 filler(), text("运费: "),
                 text("￥" + Utils::format_price(DELIVERY_PRICES[del_idx])) |
                     color(Color::Blue)}));
            rows.push_back(
                hbox({text(" 送达地址: "),
                      text(std::string(full_order.address)) |
                          color(Color::Cyan) | size(WIDTH, GREATER_THAN, 20)}));
            ...
            Color border_color = is_focused ? Color::Cyan : Color::GrayLight;
            Color bg_color = is_focused ? static_cast<Color>(Color::Grey23)
        ...
        main_container->Add(item_renderer);
    }
}
